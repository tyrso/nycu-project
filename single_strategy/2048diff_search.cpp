...
/**
	...
	state select_best_move_diff(const board &b, int dep){
		...
		for(state& move : after){
			if(move.assign(b)){
				...
				float tmp=tree_search_popup(now,dep,best_value)+move.reward();
	...
	float tree_search_popup(const board& b,int dep,float mxval){
		float score = 0,num=0;
		int n = 0;
		for(int i=0;i<16;i++) if(!b.at(i)) n++;
		for(int i=0;i<16;i++){
			if(!b.at(i)){
				num++;
				int r=rand()%(int)n;
				if(r > diff_cnt) continue; 
	...
	float tree_search_move(const board&b,int dep,float mxval){
		...
		for(state& move : after){
			if(move.assign(b)){
				...
				float tmp = tree_search_popup(now,dep,mxval) + move.reward();
...
public:
	const float worst_rate = 0.7;
	const float diff_rate  = 0.3;
	const float diff_cnt   = 5;
	const int ply1=0,ply3=0,ply5=150000;
};
#define lrn learning()
int score_to_ply(int &score){
	//if(score < ply1) return 0;
	//if(score < ply3) return 1;
	if(score < lrn.ply5) return 2;
	return 3;
}
int main(int argc, const char* argv[]) {
	...
	info << "ply1 = " << lrn.ply1 << std::endl;
	info << "ply3 = " << lrn.ply3 << std::endl;
	info << "ply5 = " << lrn.ply5 << std::endl;
	info << "ply7 = " << "inf" << std::endl;
    info << "worst_rate = " << lrn.worst_rate << std::endl;
	info << "diff_rate = " << lrn.diff_rate << std::endl;
	info << "diff_cnt =  " << lrn.diff_cnt << std::endl;
	std::srand(seed);
	...
	for (size_t n = 1; n <= total; n++) {
		...
		while (true) {
			...
			state best = tdl.select_best_move_diff(b,score_to_ply(score));//todo(create new test function)
			path.push_back(best);
