...
public:
	...
	state select_best_move_abcut(const board &b, int dep=1){
		//TODO
	...
	float tree_search_popup(const board& b,int dep,float mxval){
		float score = 0,num=0,worst_rate=0.8;
		float L,U;
		for(int i=0;i<16;i++){
			if(!b.at(i)){
				num++;
...
int main(int argc, const char* argv[]) {
	...
	for (size_t n = 1; n <= total; n++) {
		...
		while (true) {
			...
			state best = tdl.select_best_move_abcut(b,ply);//todo(create new test function)
			path.push_back(best);
